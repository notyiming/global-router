"""Global Router Command Line Interface."""
...
@gr.command(short_help="Route the netlist")
...
@click.option(
    ...
    "--algorithm",
    default=1,
    help="1. Best First Search 2. Breadth First Search",
    type=int,
)
@click.option("-s", "--seed", default=-1, help="Random seed", type=int)
def global_route(
    input_file: str, output_file: str, algorithm: int, seed: int | None
) -> Tuple[Dict, GlobalRouter]:
    """Global Route a netlist file and generate a routed output.

    Attributes
    ----------
    input_file: str
        Path to netlist input file.
    output_file: str:
        Path to generated output file.
    algorithm: int
        Routing algorithm. Defaults to 1.
    seed :int
    ...
    -------
    Tuple[Dict, GlobalRouter]
        Netlist details and the best global router.
    ...
    with Live(_generate_table(), auto_refresh=False) as live:
        ...
        for i in range(num_threads):
            ...
            if router.overflow < min_overflow or (
                router.overflow == min_overflow and router.wirelength < min_wirelength
            ):
    ...
    if min_overflow > 0:
        ...
        while num_of_reroutes < num_of_reroutes_attempts and min_overflow > 0:
            ...
            new_min_overflow, new_min_wirelength = global_routers[
                best_gr_index
            ].rip_up_and_reroute()
            if new_min_overflow < min_overflow or (
                new_min_overflow == min_overflow and new_min_wirelength < min_wirelength
            ):
...
def _generate_table(gr_results=None) -> Table:
    ...
    if gr_results is not None:
        for res in gr_results:
            gr_table.add_row(
                f"{res['id']}", f"{res['best_overflow']}", f"{res['best_wirelength']}"
            )
    return gr_table
...
@click.option(
    "-d", "--display_plot_from_cli", is_flag=True, help="Display plot from CLI"
)
def plot_congestion(congestion_data_file_path: str, display_plot_from_cli=False) -> str:
    ...
    Attributes
    ----------
    congestion_data_file_path: str
        Congestion data file path
    display_plot_from_cli: bool
        Display plot from CLI. Defaults to False.
    ...
    with open(congestion_data_file_path, "r", encoding="utf-8") as data:
        ...
        colors = np.select(
            [
                congestion_data == 0,
                congestion_data <= 0.25,
                congestion_data <= 0.5,
                congestion_data <= 0.75,
                congestion_data <= 1.0,
                congestion_data > 1.0,
            ],
        ...
        x = np.concatenate(
            [
                np.array(
                    [0.15 + int(i % (grid_hor - 1)) * 0.6 for i in range(num_hor)]
                ),
                np.array(
                    [
                        int((i - num_hor) % (grid_hor)) * 0.6
                        for i in range(num_hor, len(congestion_data))
                    ]
                ),
            ]
        )
        y = np.concatenate(
            [
                ...
                np.array(
                    [
                        0.15 + int((i - num_hor) / (grid_hor)) * 0.6
                        for i in range(num_hor, len(congestion_data))
                    ]
                ),
            ]
        ...
        with click.progressbar(
            range(len(congestion_data)), label="Generating Congestion Plot"
        ) as data:
            for i in data:
                ax.add_patch(
                    patches.Rectangle(
