...
class GlobalRouter:
    ...
    def dump_result(self, output_file_path: str) -> None:
        ...
        with open(output_file_path, file_mode) as output:
    ...
    def generate_congestion_output(self, output_file_name: str) -> None:
        ...
        """
        with open(f"{output_file_name}.fig", "x") as output:
            output.write(f"{self.grid_horizontal_size} {self.grid_vertical_size}\n")
            for i in range(self.number_of_edges):
                output.write(f"{self.demand[i]/(self.horizontal_capacity if i < self.number_of_horizontal_edges else self.vertical_capacity)} ")
    ...
    def parse_input(self, input_file_path: str) -> List[Net]:
        ...
        with open(input_file_path, "r") as input:
            ...
            self.netlist_size = int(input.readline().split()[2])
            ...
            self.number_of_horizontal_edges = (self.grid_horizontal_size - 1) * self.grid_vertical_size
            self.number_of_edges = self.number_of_horizontal_edges + (self.grid_vertical_size - 1) * self.grid_horizontal_size
