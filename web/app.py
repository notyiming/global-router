...
def register():
    ...
    if request.method == "POST":
        ...
        try:
            ...
            encoded_email = (
                base64.urlsafe_b64encode(email.encode()).decode().rstrip("=")
            )
...
def download(filename):
    ...
    if "user" not in session:
        return redirect("/login")
    encoded_email = (
        base64.urlsafe_b64encode(session["user"].encode()).decode().rstrip("=")
    )
...
def dashboard():
    ...
    if request.method == "POST":
        ...
        netlist_details, global_router = cli.global_route.callback(
        ...
        result = {
            ...
            "fig_html": cli.plot_congestion.callback(f"output/{file_basename}.out.fig"),
        ...
        storage.child(f"{session['user']}/{result['unique_name']}").put(
            f"output/{file_basename}.out"
        )
    ...
    return render_template(
        "dashboard.html", user=user, outputs=outputs.values() if outputs else []
    )
